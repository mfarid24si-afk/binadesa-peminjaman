    ...
    <div class="main-wrapper mdc-drawer-app-content">
      ...
      <div class="page-wrapper mdc-toolbar-fixed-adjust">
        <main class="content-wrapper">
            <div id="pembayaran" class="tab-content {{ request()->routeIs('petugas') ? 'active' : '' }}">
                {{-- ==================== TABLE Petugas ==================== --}}
            <div class="mdc-layout-grid__cell stretch-card mdc-layout-grid__cell--span-12">
              <div class="mdc-card p-0">
                <h6 class="card-title card-padding pb-0">Data Petugas</h6>
                <div class="table-responsive">
                                                           <form method="GET" action="{{ route('petugas') }}" class="mb-3">
    @csrf
    <div class="row">
        <div class="col-md-2">
            <select name="peran" class="form-select" onchange="this.form.submit()">
                <option value="">Peran</option>
                <option value="Penanggung Jawab" {{ request('deskripsi')=='Penanggung Jawab' ? 'selected' : '' }}>Penanggung Jawab</option>
                <option value="Operator" {{ request('peran')=='Operator' ? 'selected' : '' }}>Operator</option>
                <option value="Petugas Kebersihan" {{ request('peran')=='Petugas Kebersihan' ? 'selected' : '' }}>Petugas Kebersihan</option>
                <option value="Pengelola" {{ request('peran')=='Pengelola' ? 'selected' : '' }}>Pengelola</option>
                <option value="Koordinator" {{ request('peran')=='Koordinator' ? 'selected' : '' }}>Koordinator</option>
            </select>
        </div>
